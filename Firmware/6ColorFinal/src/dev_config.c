...
static int dev_configure_raw_low(const struct gpio_dt_spec *spec) {
  if (!spec || !device_is_ready(spec->port)) {
    return -ENODEV;
  }

  return gpio_pin_configure(spec->port, spec->pin, GPIO_OUTPUT_LOW);
}

static int dev_disconnect_pin(const struct gpio_dt_spec *spec) {
  if (!spec || !device_is_ready(spec->port)) {
    return -ENODEV;
  }

  return gpio_pin_configure(spec->port, spec->pin, GPIO_DISCONNECTED);
}

static void dev_store_first_error(int *err, int ret) {
  if (*err == 0 && ret != 0) {
    *err = ret;
  }
}
...
int DEV_Module_Exit(void) {
  int ret = 0;

  dev_store_first_error(&ret, dev_configure_raw_low(&epd_dc));
  dev_store_first_error(&ret, dev_configure_raw_low(&epd_cs));
  dev_store_first_error(&ret, dev_configure_raw_low(&epd_rst));

#if DEV_CONFIG_HAS_POWER
  dev_store_first_error(&ret, dev_configure_raw_low(&epd_power));
#endif

  dev_store_first_error(&ret, dev_disconnect_pin(&epd_dc));
  dev_store_first_error(&ret, dev_disconnect_pin(&epd_cs));
  dev_store_first_error(&ret, dev_disconnect_pin(&epd_rst));
