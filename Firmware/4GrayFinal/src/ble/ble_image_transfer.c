...
#define IMAGE_TRANSFER_BYTES EPD_4GRAY_IMAGE_BYTES
...
static void display_work_handler(struct k_work *work)
{
	...
	ret = epd_display_4gray(image_buf);
	if (ret != 0) {
	...
	ret = epd_hibernate();
	if (ret != 0) {
		LOG_ERR("Failed to hibernate EPD: %d", ret);
	}
			  ...
			  uint8_t flags)
{
	...
	image_expected = val;
	image_received = 0U;
	LOG_INF("4-gray image transfer started: %u bytes", val);
			  ...
			  uint8_t flags)
{
	...
	if (image_received == image_expected) {
		LOG_INF("4-gray image transfer complete");
